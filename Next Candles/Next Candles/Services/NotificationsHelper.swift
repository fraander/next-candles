...
class NotificationsHelper: ObservableObject {
    ...
    func scheduleNotification(name: String, dateComponents: DateComponents, distanceFromBD: Int, id: String, hour: Int, minute: Int) async throws -> String {
        ...
        let body = (distanceFromBD == 0) ? "\(name.last == "s" ? name + "'" : name + "'s") birthday is today, \(birthdate.formatted(.dateTime.month(.wide).day(.twoDigits)))." : "\(name.last == "s" ? name + "'" : name + "'s") birthday is on \(df.string(from: birthdate)), which is \(distanceFromBD) \(distanceFromBD == 1 ? "day" : "days") away."
